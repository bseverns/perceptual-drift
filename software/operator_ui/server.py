...
def _error_response(
    handler: BaseHTTPRequestHandler, message: str, status: int
) -> None:
    ...
    class OperatorHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if parsed.path.startswith("/api/"):
                if (
                    self._requires_auth(parsed.path)
        ...
        def do_POST(self) -> None:
            ...
            if not parsed.path.startswith("/api/"):
                _error_response(
                    self,
                    "POST only allowed for /api endpoints",
        ...
        def log_message(
            self, fmt: str, *args
        ) -> None:  # quiet default handler
            return

        def _requires_auth(self, api_path: str) -> bool:
            if not api_token:
                return False
            return api_path not in self._PUBLIC_API_PATHS
        ...
        def _is_authorized(self) -> bool:
            ...
            return secrets.compare_digest(provided_token, api_token)
        ...
        def _handle_api_get(self, parsed) -> None:
            ...
            if method_name:
                ...
                _error_response(
                    self, f"unknown endpoint: {path}", HTTPStatus.NOT_FOUND
                )
        ...
        def _api_post_rehearsal_start(self, _parsed, payload) -> None:
            profile_id = str(
                payload.get("profile_id", "safe_synthetic")
            ).strip()
            notes = str(payload.get("notes", "")).strip()
            label = str(payload.get("label", "")).strip()
            if not label:
        ...
        def _handle_api_post(self, parsed) -> None:
            ...
            try:
                ...
                _error_response(
                    self, "invalid JSON body", HTTPStatus.BAD_REQUEST
                )
                return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Perceptual Drift operator UI server"
    )
    ...
    parser.add_argument(
        ...
        help="Base mapping path",
    )
    parser.add_argument(
        "--recipes-dir", default="config/recipes", help="Recipes dir"
    )
    parser.add_argument(
        "--static-dir",
    ...
    parser.add_argument(
        ...
        help=(
            "Token for API endpoints (except /api/health) via "
            "Authorization: Bearer <token>."
        ),
    )
    parser.add_argument(
        "--allow-ephemeral-token",
        action="store_true",
        help=(
            "Generate a temporary token when --api-token is omitted "
            "(development only)."
        ),
    )
...
def _parse_runtime_targets(raw: str) -> list[tuple[str, int]]:
    ...
    for part in raw.split(","):
        ...
        if ":" not in text:
            raise ValueError(
                f"invalid runtime target '{text}' (expected host:port)"
            )
...
def main() -> int:
    ...
    if not api_token:
        if not args.allow_ephemeral_token:
            print(
                (
                    "[operator-ui] ERROR: missing API token. Set "
                    "OPERATOR_API_TOKEN or pass --api-token. "
                    "Use --allow-ephemeral-token only for local development."
                ),
                file=sys.stderr,
            )
            return 2
        ...
        print(
            "[operator-ui] WARNING: generated temporary API token: "
            f"{api_token}"
        )
    ...
    handler = make_handler(
        state, static_root, supervisor, preflight_runner, api_token
    )
